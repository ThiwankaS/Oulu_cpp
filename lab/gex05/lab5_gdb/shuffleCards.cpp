...
void shuffleCards(){
	...
	for (int i = 1; i <= num_cards; i++) {
        std::string filename = INPUT_FILE + std::to_string(i) + ".png";
        PNGLayer card(WIDTH, HEIGHT, Vector2(0.0, 0.0), Vector2(1.0, 1.0), RGBAPixel(255,255,255,255), std::to_string(i));
        card.readFromFile(filename);
	...
	for (auto& card : list) {
        card.SetPosition(Vector2(WIDTH/100 + (WIDTH - WIDTH/9) / num_cards * index, HEIGHT / 4));
        outputCanvas.Add(card);
        index++;
    }

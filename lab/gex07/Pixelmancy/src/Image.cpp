...
bool Image::operator==(const Image& other) const
{
    ...
    if (m_colorPalette.getColors() != other.m_colorPalette.getColors())
    {
        return false;
    }
...
Image Image::resize(double percentage) const
{
    ...
    for (int j = 0; j < height; ++j)
    {
        for (int i = 0; i < width; ++i)
        {
            ...
            originalX = std::min(originalX, m_imageDimensions.width - 1);
            originalY = std::min(originalY, m_imageDimensions.height - 1);

        ...
        for (std::size_t i = 0; i < 3; i++)
        {
            ...
            for (auto& dObj : dObjs)
            {
                ...
                if (dObj->getObjectType() == pixelmancy::graphics::ObjectType::CIRCLE)
                {
                    ...
                    pos = pixelmancy::graphics::Point(pos.x + square_width / 2, pos.y + square_height / 2);
                }
...
int drawOnImage(std::shared_ptr<pixelmancy::ColorMatcher> colorMatcher)
{
    ...
    srand(42);
    std::vector<pixelmancy::graphics::Point> positions;
    positions.reserve(70);
    for (int j = 0; j < 70; j++)
    {
    ...
    pixelmancy::ColorPallette colorPallette = smallImage.getColorPalette();
